...
function discountAppliedLabel(order) {
  ...
  function orderItemPrintMode(item) {
    if (!item || String(item.pricingMode || '').toLowerCase() === 'large_format') return '';
    const labelText = `${item.priceRuleLabel || ''} ${item.selectionLabel || ''}`;
    const isBooklet = item.booklet === true || /\(\s*booklet\s*\)/i.test(labelText);
    if (isBooklet) return 'booklet';
    if (item.fb === true || /(?:\(|\b)f\/b(?:\)|\b)/i.test(labelText)) return 'fb';
...
<div class="modal fade" id="ordersPayConfirmModal" tabindex="-1" aria-labelledby="ordersPayConfirmModalLabel" aria-hidden="true">
            ...
            <div class="list-group-item d-flex align-items-start justify-content-between" style="padding:0.5rem 0.75rem;">
              <div style="flex:1;min-width:0;">
                <div class="d-flex align-items-center gap-2">
                  ...
                  ${printModeBadgeHtml(row.printMode || (row.booklet ? 'booklet' : (row.fb ? 'fb' : '')))}
  ...
  function receiptItemData(it) {
    ...
    return {
      ...
      fb: !!(isFb && !isBooklet),
      booklet: !!isBooklet,
      printMode: orderItemPrintMode(it),
      subtotal
    };
  }
      ...
      const rows = group.items.map(row => {
        ...
        return `
          <tr>
            ...
            <td>
              <div class="receipt-muted receipt-selection">${printToneBadgeHtml(row.tone)}${printModeBadgeHtml(row.printMode || (row.booklet ? 'booklet' : (row.fb ? 'fb' : '')))}<span>${escapeHtml(row.selectionLabel)}</span></div>
